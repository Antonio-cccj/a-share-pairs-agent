...
def test_signal_opens_on_extreme_z():
    ...
    a = pd.Series(100 + np.cumsum(rng.normal(0, 1, n)), index=pd.bdate_range("2020-01-01", periods=n))
    ...
    pair = PairCandidate(
        code_a="A", code_b="B", industry="X",
        pvalue=0.001, hedge_ratio=1.0, intercept=5.0,
        half_life=10.0, adf_stat=-3.5,
    )
